...
def BB84(n, noise_level=0.0, intercept=False, verbose=True):
    ...
    # QBER = errori / num di volte basi uguali
    QBER = bit_error / len(match)
...
def testBB84(n, run):
    errors = 0
    for i in range(run):
        errors = errors + BB84(n, 0.1,True, False)
    print("Il protocollo ha fallito nel " + str(errors * 100 / run) + "% dei casi")


n_bit = 20
n_run = 4
BB84(n = n_bit, noise_level=0.1, intercept=False, verbose=True)
